...
class Soldier(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, speed, ammo, grenades, ):
        ...
        for animation in animation_types:
            ...
            for i in range(num_of_frames):
                img = pygame.image.load(f"images/{self.char_type}/{animation}/{i}.gif").convert_alpha()
    ...
    def ai(self):
        if self.alive and player.alive:
            ...
            if self.vision.colliderect(player.rect) and self.rect.bottom == player.rect.bottom and self.move_counter != 1:
            ...
            for x, tile in enumerate(row):
                if tile >= 0:
                    ...
                    if 0 <= tile <= 8:
                        ...
                        player = Soldier("player", x * TILE_SIZE, y * TILE_SIZE, 0.04, 5, 20, 5)
                        health_bar = Health_Bar(10, 10, player.health, player.health)
                    elif tile == 16:  # create enemy
                        enemy = Soldier("enemy", x * TILE_SIZE, y * TILE_SIZE, 0.8, 3, 20, 0)
                        ...
                        exit_group.add(exit)
                    elif tile == 21:  # create decoration
                        decoration = Decoration(img, x * TILE_SIZE, y * TILE_SIZE)
                        decoration_group.add(decoration)
...
while run:
    ...
    if not start_game:
        ...
        for x in range(player.grenades):
            screen.blit(grenade_img, (130 + (x * 20), 58))
